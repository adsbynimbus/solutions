...
suspend fun BannerAdRequest.Builder.loadAsync(): BannerAd = suspendCancellableCoroutine { coroutine ->
    val callback = object : AdLoadCallback<BannerAd> {
        override fun onAdFailedToLoad(adError: LoadAdError) {
            if (coroutine.isActive) coroutine.resumeWithException(RuntimeException(adError.message))
        }

        override fun onAdLoaded(p0: BannerAd) {
            if (coroutine.isActive) coroutine.resume(p0)
        }
    }
    val request = build()
    Log.i("Ads", "[${request.adUnitId}][${adSize.formatString}] [${request.customTargeting.entries.joinToString(", \n")}]")
    BannerAd.load(request, callback)
}
